...
class DataFlowAnalysis {
  ...
  protected:
		...
		void initializeBackwardMap(Function * func) {
			assignIndiceToInstrs(func);
//            InstrToIndex.erase(nullptr);
//            IndexToInstr.erase(0);
...
//			TODO: Add exit instr. Check correctness
...
//		for (auto const &it : IndexToInstr) {
//			if (it.first==0)
//				continue;
...
//				newInfo = new Info();
//				Edge outgoingEdge =std::make_pair(n, outgoingEdges[i]);
//				newInfo->join(infos[i], EdgeToInfo[outgoingEdge], newInfo);
//				if (! newInfo->equals(EdgeToInfo[outgoingEdge], newInfo)){
